...
python3 $W/make-served-census-receipt-r216.py; echo "receipt exit=$?" || exit 1
echo "### record t216 serving state $(date -u +%FT%TZ)"
docker logs glm53-t216 > $W/t216-serve-final.log 2>&1; echo "log exit=$?"
docker inspect glm53-t216 > $W/t216-container-inspect-pre-stop.json; echo "inspect exit=$?"
echo "### stop glm53-t216 (docker stop; container preserved, not removed)"
docker stop glm53-t216; echo "stop exit=$?"
sleep 5
docker ps --format '{{.Names}} {{.Status}}'
echo "### launch glm53-r216 $(date -u +%FT%TZ)"
