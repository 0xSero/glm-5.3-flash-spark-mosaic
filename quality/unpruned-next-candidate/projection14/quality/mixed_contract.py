"""Fail-closed validation of the exact assembled projection14 candidate."""
import hashlib,json,re,struct
from pathlib import Path
MANIFEST_SHA='80967e71922a534a3ab2872ad90edc4aacaf958bcb0a060699e599318727556d'
...
def expected_bits(prefix):
 ...
 return 3 if layer in (5,6,7,20,22,23,26,27,28,31,32,35,36,37) and m[3]=='down_proj' else 2
...
def validate_candidate(root,experts):
 ...
 require(sha(mp)==MANIFEST_SHA,'not sealed projection14 manifest')
 m=json.loads(mp.read_text());s=json.loads((root/'BUILD_STATUS.json').read_text())
 require(experts==288 and m['schema']=='glm53-mixed-projection-exl3-v1' and m['state']=='STRUCTURAL_PASS','mixed source contract mismatch')
 ...
 require(m['layer_projection_bits']=={str(l):{'down_proj':3} for l in (5,6,7,20,22,23,26,27,28,31,32,35,36,37)} and m['default_bits']==2,'incorrect precision map')
 require(m['native_mtp_experts']==288 and m['protected_tensor_bytes']==33835039608,'native protected closure mismatch')
 require(m['metadata_sha256']['protected-tensor-closure.json']=='58d00c3cc754a12980a21d21855d33b3a00367134562a2d13967215a1b489aba' and m['metadata_sha256']['source-config.json']=='6f01b4d22b1cfd19f0f6892356bd88fc2725f443a3fd29d8003f3981cc338ecf','original native closure identity changed')
 ...
 require(c['quantization_config']==q and q['bits']==2 and q['layer_projection_bits']==m['layer_projection_bits'] and q['rank_stacked_tp']==4,'runtime precision metadata differs')
 ...
 require(total==115502651256 and protected==33835039608,'tensor payload closure mismatch')
 return MANIFEST_SHA
