"""Bounded projection14 preflight/capture; never changes existing sources/jobs."""
import argparse,json,subprocess,time,shutil
from pathlib import Path
W=Path(__file__).resolve().parent;ROOT=W.parents[3];MODEL=W.parent/'model';IMAGE='sha256:54acf16064726697ab079dfbd297c1c1c260cb28867f8287f5ae2ec3bbb0a382';RESULT='/release/results/projection14-k2-gateup-k3-down'
...
if gpu:
 release=json.loads((W/'ROOT_GPU_RELEASE.json').read_text());assert release['state']=='ROOT_EXPLICITLY_RELEASED_PROJECTION14_QUALITY'
...
cmd=['docker','run','-d','--name','glm53-projection14-quality-'+a.phase,'--network','none','--cpus','4','--memory','64g' if gpu else '8g','--memory-swap','64g' if gpu else '8g']
...
if gpu:
 ...
 cmd=['docker','run','--name','glm53-projection14-quality-seal','--network','none','--cpus','2','--memory','2g','--memory-swap','2g','--entrypoint','python3','-v',str(ROOT)+':/release:ro','-v',str(W)+':/quality',IMAGE,'/quality/seal_quality_result.py','--result',RESULT,'--inspect','/quality/capture-inspect.json','--history','/quality/layer-history.json','--output','/quality/quality-integrity-seal.json']
