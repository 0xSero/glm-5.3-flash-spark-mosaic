#!/usr/bin/env python3
"""Immutable CPU projection14 assembly; all target expert IDs/router rows retained."""
...
SELECTION_SHA='57926af4ddf6aa5f5cf0180ec6cf57b867f4ec62457d806ecab1310cddf65c78'
...
def main():
 ...
 sel=H.read(HERE/'selection.json');selected=set(map(int,sel['layer_projection_bits']));assert selected=={32, 35, 36, 5, 6, 7, 37, 20, 22, 23, 26, 27, 28, 31}
 ...
 prior=HERE.parent/'projection/model';prior_manifest=prior/'EXL3_MANIFEST.json'
 if H.sha(prior_manifest)!='590ec08e84a66767e03964f7482e84c952e6b98c26cddc7d45983592c4e491f5':raise ValueError('projection6 identity mismatch')
 prior_files={f['path'].removeprefix('weights/'):f for f in H.read(prior_manifest)['files']};reused={5,26,31,32,35,36}
 out=HERE/'model';assert not out.exists()
 required=0
 for rec in inv['files']:
  m=re.fullmatch(r'layer-(\d+)-part-tail-(\d+)\.safetensors',rec['path'])
  if m and int(m[1]) in selected and int(m[1]) not in reused:required+=rec['bytes']+144*1048576
  elif Path(mapping[rec['sha256']]).stat().st_dev!=HERE.stat().st_dev:required+=rec['bytes']
 if shutil.disk_usage(HERE).free<required+10*2**30:raise ValueError('insufficient build/disk floor')
 ...
 for i,rec in enumerate(inv['files'],1):
  ...
  if m and int(m[1]) in selected:
   ...
   if layer in reused:
    pr=prior_files[rec['path']];assert pr['sources']==contributors+[{'tier':3,'sha256':rec3['sha256'],'projection':'down_proj'}];mode=H.verified_materialize(prior/pr['path'],dst,pr);replaced_fields+=144*4*4
   else:replaced_fields+=rewrite(src2,src3,dst,rec,rec3);mode='projection_rewrite'
   contributors.append({'tier':3,'sha256':rec3['sha256'],'projection':'down_proj'})
