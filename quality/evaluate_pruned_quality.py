#!/usr/bin/env python3
"""Full-trunk REAP quality using the unchanged low-bpw BF16 head comparison.
...
def validate_candidate_files(root, experts):
    ...
    if manifest.get('schema') != 'glm53-physical-reap-exl3-v1':
        raise ValueError('pruned mode requires physical REAP manifest; original source requires --source-inventory')
    status = json.loads((root / 'BUILD_STATUS.json').read_text())
    if (manifest.get('schema') != 'glm53-physical-reap-exl3-v1' or manifest.get('state') != 'STRUCTURAL_PASS'
            or manifest.get('keep') != experts or manifest.get('native_mtp_n_routed_experts') != 288
            or status.get('state') != 'COMPLETE' or status.get('manifest_sha256') != sha(manifest_path)):
        raise ValueError('candidate lacks bound structural proof and preserved native MTP')
    required = {'BUILD_CONTRACT.json', 'PROTECTED_TENSORS.json', 'config.json',
                'quantization_config.json', 'source-config.json', 'model.safetensors.index.json'}
    metadata = manifest.get('metadata_sha256', {})
    if set(metadata) != required:
        raise ValueError('candidate structural metadata coverage mismatch')
    ...
    for item in manifest.get('files', []):
        name = item['path']
        if item.get('omitted_empty_shard'):
            if (root / name).exists():
                raise ValueError('supposedly omitted shard exists')
            continue
        if name in observed:
    ...
    if indexed != observed:
        raise ValueError('candidate manifest and indexed shard set disagree')
...
def main():
    ...
    if args.source_inventory:
        ...
        artifact_mode = 'physical_reap_candidate'
    dependencies = ('release_route_capture_v2', 'glm53_exl3_tp4')
    identity = {'schema': 'glm53-pruned-full-trunk-quality-identity-v1',
    ...
    if args.phase in ('compare', 'all'):
        ...
        report = {'schema': 'glm53-pruned-full-trunk-quality-v1', 'state': 'QUALITY_MEASURED',
        ...
        report['schema'] = 'glm53-pruned-full-trunk-quality-v1'
