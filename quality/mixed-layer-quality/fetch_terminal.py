...
W=Path(__file__).resolve().parent;REMOTE='/home/valentine/glm53-single-spark-release-20260911/quality/mixed-layer-quality'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
while True:
 check=subprocess.run(['ssh','-o','ConnectTimeout=10','spark-de5c','cat',REMOTE+'/seal-status.json'],capture_output=True,text=True,timeout=20)
 if check.returncode==0:
  status=json.loads(check.stdout)
  if status['state']=='INTEGRITY_SEALED_QUALITY_NOT_ACCEPTED':break
 # Do not interpret an unavailable status file as successful completion.
 state=subprocess.run(['ssh','-o','ConnectTimeout=10','spark-de5c','cat',REMOTE+'/status.json'],capture_output=True,text=True,timeout=20)
 ...
 time.sleep(30)
files=['quality-report.json','evaluation-identity.json','normalized-manifest.json','quality-integrity-seal.json','seal-status.json','capture-inspect.json','layer-history.json','capture.log','seal.log']
...
assert r['candidate_manifest_sha256']=='73291ede5c0f2ad13dcd9fbdfe3887f133ef713d56c589c59c6392bdb887b45c'
