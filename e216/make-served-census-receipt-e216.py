#!/usr/bin/env python3
"""E216 served census receipt (pre-staged BEFORE the first docker run — the T216 attempt-1 trap).

Same established transformation as U216/S216/T216/R216: the raw plan-aware census records the stock
...
verdict.contract_ok true. Produce the cleared copy at /tmp/e216-receipts/exl3-plain-census-2p05.json;
raw receipt preserved untouched.
"""
import json, re
from pathlib import Path
raw_path = Path("/home/valentine/e216/census-pruned-e216-plan-aware.json")
...
served["served_receipt_note"] = ("problems cleared per established U216/S216/T216/R216 served-copy convention "
                                 f"(42 expected 216-vs-288 deviations verified against plan {raw.get('plan_sha256','')[:16]}…; "
                                 "raw plan-aware census preserved at e216/census-pruned-e216-plan-aware.json); loader checks: "
                                 "schema, index_only, verdict.contract_ok")
body = json.dumps(served, indent=1, sort_keys=True) + "\n"
Path("/tmp/e216-receipts").mkdir(parents=True, exist_ok=True)
for dst in ("/home/valentine/e216/exl3-plain-census-2p05.json", "/tmp/e216-receipts/exl3-plain-census-2p05.json"):
