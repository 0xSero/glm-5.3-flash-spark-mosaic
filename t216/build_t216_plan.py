#!/usr/bin/env python3
"""T216 plan: UNIFORM keep-216 prune of the 3.05bpw base ranked by K3 sensitivity ALONE.

Hypothesis (queue item 2): S216's route factor adds noise — corr(sensitivity, ln routes) ≈ 0.09 per layer means
the combined score is sensitivity-dominated anyway, and the route capture was thin on generated tokens, so drop it
from the score and keep the 216 highest-sensitivity experts per layer. sensitivity[l][e] is inherited from the SEALED
S216 plan (sensitivity_by_layer, which build_s216_plan.py derived from k3-projection-errors.json 89a26929…); this
script refuses to run if the local cross-check copy (sensitivity.json) disagrees by more than 1e-12 on any expert.
Routing counts are loaded only for the same auxiliary stats the S216 plan records (retention, zero-route experts)
and for keep-set overlaps; they do NOT enter the score. Schema matches the S216/U216 plans so prune_u216.py /
verify_u216.py / census_plan_aware.py apply unchanged. MTP layer 45 is kept verbatim.
"""
...
ap.add_argument('--routing', required=True, help='routing-counts.json, sha-verified against the sealed pin; stats only')
...
# plan_sha256 covers the canonical body WITHOUT the plan_sha256 field (same convention as build_s216_plan.py)
...
require(sha(a.routing) == '1f0e98c8d782f067ae575e7570074f5aec3249a481086da08912a5faed05bebb', 'routing-counts.json sha != sealed pin 1f0e98c8…')
counts = rt['counts']
...
ranking = {l: sorted(range(N), key=lambda e: (-sens[l][e], e)) for l in LAYERS}
...
for l in LAYERS:
    ...
    detail[str(l)] = {'keep': K, 'route_mass_retention': rk / ra, 'sensitivity_retention': sk / sa, 'zero_route_experts': zero,
                      ...
                      'score_threshold': sens[l][ranking[l][K - 1]]}
...
plan = {
    'schema': 'dynamic-container-plan-v1', 'name': 'T216', 'state': 'FROZEN_PLAN',
    'point': {'bpw': 3.05, 'prune': removed / (N * len(LAYERS)), 'removed_experts': removed, 'retained_experts': K * len(LAYERS),
              'mean_keep': K, 'ranking': 'k3_sensitivity_only', 'uniform_keep': K, 'layer_floor': K, 'block': None,
              'removal_count_source': f'uniform keep {K} per MoE layer x {len(LAYERS)} layers (SGLang requires a single n_routed_experts)'},
    'base': s216['base'],
    'inputs_sha256': {'s216-plan.json': sha(a.s216_plan), 'routing-counts.json': sha(a.routing),
                      'provenance_note': f'sensitivity_by_layer inherited verbatim from the sealed S216 plan (sha {s216["plan_sha256"][:16]}…), '
                                         f'whose inputs_sha256 pins k3-projection-errors.json {sealed_pin[:16]}…; independent reduced copy '
                                         'sensitivity.json cross-checked at max |diff| 0.0 over 12,096 experts; routes excluded from the score by design'},
    'routing_capture': {k: rt[k] for k in ('total_routes', 'per_batch_routes', 'receipt') if k in rt} | {'source_model': 'turboderp/GLM-5.3-Flash-exl3 2.05bpw (unpruned) served by exl3_plain on spark-2384',
                        'note': 'recorded for retention/overlap stats only; the T216 score does not use routes (hypothesis: the ln(routes+1) factor is noise at the current capture depth)'},
    'allocation_proxy': 'none (uniform); within-layer ranking = mean(K3 gate/up/down proxy error) alone',
    'allocator': 'uniform top-K of sensitivity, ties lower id',
    ...
    'scores_by_layer': {str(l): sens[l] for l in LAYERS}, 'sensitivity_by_layer': {str(l): sens[l] for l in LAYERS}, 'routes_by_layer': {str(l): routes[l] for l in LAYERS},
