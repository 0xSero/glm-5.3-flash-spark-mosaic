...
echo "### fixup symlinks (attempt1 used ../panel relative to cwd=/ -> dangling literal link; compare FileNotFoundError preserved in g4-t216.out)"
B -c "rm -f '/w2port/g4/panel-t216/teacher-row-*.pt' && ln -sf /w2port/g4/panel/teacher-row-*.pt /w2port/g4/panel-t216/ && ls /w2port/g4/panel-t216/ | grep -c 'teacher-row-[0-9]'"; echo "symlink exit=$?"
echo "### compare (retry) $(date -u +%FT%TZ)"
...
echo "T216_G4_FIXUP_DONE $(date -u +%FT%TZ)"
