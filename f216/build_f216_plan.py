#!/usr/bin/env python3
"""F216 plan: UNIFORM keep-216 prune of the 3.05bpw base ranked by the repo-verbatim REAP score
computed on the FULL (SEALED_COMPLETE) observation dataset.

User goal 2026-09-15. Same score as E216 (weighted_ean_sum / expert_frequency = the repo's reap,
pruning_metrics.py:198 with top-k renormalized router weights) but on the COMPLETED observation
dataset: science 195 records + cuda 1,645 records added (1,840 records / ~4.73M tokens), seal
state SEALED_COMPLETE_ORIGINAL_RECORDS, all 361 shards committed contiguously. Requires the full
seal's coverage to equal the contract (23,088 records / 37,328,459 tokens). MTP layer 45 verbatim.
"""
...
require(obsw.get('state') == 'SEALED_COMPLETE_ORIGINAL_RECORDS', f"unexpected seal state {obsw.get('state')} — F216 requires the COMPLETED observation dataset")
cov_r = obsw.get('covered_domain_records', {}); cov_t = obsw.get('covered_domain_tokens', {})
require(sum(cov_r.values()) == 23088, f"covered records {sum(cov_r.values())} != contract 23088")
require(sum(cov_t.values()) == 37328459, f"covered tokens {sum(cov_t.values())} != contract 37328459")
require('science' in cov_r and cov_r['science'] == 195, f"science domain missing/incomplete: {cov_r.get('science')}")
require('cuda' in cov_r and cov_r['cuda'] == 2000, f"cuda domain incomplete: {cov_r.get('cuda')}")
...
plan = {
    'schema': 'dynamic-container-plan-v1', 'name': 'F216', 'state': 'FROZEN_PLAN',
    ...
    'inputs_sha256': {'observations.json': sha(a.observations), 'routing-counts-r216.json': sha(a.routing),
                      ...
                      'provenance_note': 'REAP-native score per expert = weighted_ean_sum / expert_frequency from the sealed 0xSero observations, '
                                         'COMPLETED dataset (schema glm53-original-records-seal-v1, state SEALED_COMPLETE_ORIGINAL_RECORDS: '
                                         '23,088/23,088 records incl. science 195 + cuda completed; 37,328,459 tokens; all 361 shards committed '
                                         'contiguously; prior partial seal 21,248 records preserved untouched). Matches reap-strict-serialization src/reap/pruning_metrics.py:198 '
    ...
    'routing_capture': {'source_model': 'turboderp/GLM-5.3-Flash-exl3 2.05bpw (unpruned) served by exl3_plain on spark-2384',
                        'note': 'DIAGNOSTIC ONLY — the F216 score does not use route counts; totals embedded for comparability with S216/R216/E216',
