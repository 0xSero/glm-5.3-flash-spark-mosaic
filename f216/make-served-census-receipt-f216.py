#!/usr/bin/env python3
"""F216 served census receipt (pre-staged BEFORE the first docker run — the T216 attempt-1 trap).

Same established transformation as U216/S216/T216/R216/E216: the raw plan-aware census records the
stock verdict (contract_ok false) with the 42 expected 216-vs-288 deviations and problems_residual
[]; the loader (_load_census) requires schema exl3-plain-census-v1, index_only false, and
verdict.contract_ok true. Produce the cleared copy at /tmp/f216-receipts/exl3-plain-census-2p05.json;
raw receipt preserved untouched.
"""
import json, re
from pathlib import Path
raw_path = Path("/home/valentine/f216/census-pruned-f216-plan-aware.json")
...
served["served_receipt_note"] = ("problems cleared per established U216/S216/T216/R216/E216 served-copy convention "
                                 f"(42 expected 216-vs-288 deviations verified against plan {raw.get('plan_sha256','')[:16]}…; "
                                 "raw plan-aware census preserved at f216/census-pruned-f216-plan-aware.json on 2822 and on the Mac); "
                                 "loader checks: schema, index_only, verdict.contract_ok")
body = json.dumps(served, indent=1, sort_keys=True) + "\n"
Path("/tmp/f216-receipts").mkdir(parents=True, exist_ok=True)
for dst in ("/home/valentine/f216/exl3-plain-census-2p05.json", "/tmp/f216-receipts/exl3-plain-census-2p05.json"):
