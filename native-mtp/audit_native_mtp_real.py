...
def checked_get_model(*,vllm_config,model_config):
 ...
 return original_get_model(vllm_config=vllm_config,model_config=model_config)
...
report={'probe':'actual V2 draft-only real-weight load, no target weights or serving forward','production_loader':'v1.worker.gpu.spec_decode.eagle.utils.load_eagle_model','native_index_keys':len(idx),'strict_tracking':tracked,'draft_quantization':None,'draft_experts':288,'target_quantization':type(c.quant_config).__name__,'all_parameters_cuda':True,'cuda_allocated_bytes':torch.cuda.memory_allocated(),'source_value_comparisons':comparisons}
Path('/audit/native-mtp-real-load-r3.json').write_text(json.dumps(report,indent=2)+'\n')
print('REAL_LOAD_PASS',json.dumps(report),flush=True)
