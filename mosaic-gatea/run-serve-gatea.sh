#!/bin/bash
# MOSAIC Gate A serve (spark-557f). Same image + flags as the running a0 container on 2384.
set -x
W=/home/valentine/mosaic-gatea
DST=/home/valentine/models/mosaic-l20-gatea
IMG=glm53-flash-sglang-exl3-plain:serve4
CEN=$W/receipts/exl3-plain-census-mosaic-gatea.json
...
docker rm -f glm53-mosaic-gatea 2>/dev/null
docker run -d --name glm53-mosaic-gatea \
  -e EXL3_PLAIN_CENSUS=/receipts/exl3-plain-census-mosaic-gatea.json \
  ...
  --max-prefill-tokens 256 --max-running-requests 16 --mem-fraction-static 0.90 --enable-multimodal \
