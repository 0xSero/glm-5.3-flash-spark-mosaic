...
CEN=$W/receipts/exl3-plain-census-12l.json
mkdir -p $W/receipts
cp -f $W/census-mosaic-12l-stock.json $CEN 2>/dev/null
...
echo "STOP own container glm53-mosaic-10lg (10L graph test done: KV 718400, 11.4 tok/s) $(date -u +%FT%TZ)"
docker rm -f glm53-mosaic-12lh 2>/dev/null
docker rm -f glm53-mosaic-12lgh 2>/dev/null
docker rm -f glm53-mosaic-10lg 2>/dev/null
docker run -d --name glm53-mosaic-12lgh \
  ...
  --max-prefill-tokens 256 --max-running-requests 1 --mem-fraction-static 0.95 --enable-multimodal \
  --chat-template /opt/glm53/chat-template-mm.jinja --reasoning-parser glm45 --tool-call-parser glm47 \
  --cuda-graph-max-bs 1
echo "serve_launch_exit=$?"
echo "### SERVE_12LGH_LAUNCHED $(date -u +%FT%TZ)"
