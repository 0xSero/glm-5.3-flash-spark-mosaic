...
echo "STOP own container glm53-mosaic-10l (mf-0.90 KV reading = 209024 taken; panel done) $(date -u +%FT%TZ)"
docker rm -f glm53-mosaic-12lh 2>/dev/null
docker rm -f glm53-mosaic-10l 2>/dev/null
docker rm -f glm53-mosaic-10lg 2>/dev/null
docker run -d --name glm53-mosaic-10lg \
...
echo "### SERVE_10LG_LAUNCHED $(date -u +%FT%TZ)"
