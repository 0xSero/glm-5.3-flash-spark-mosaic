#!/usr/bin/env bash
# M288 mosaic as TARGET + the program's native MTP draft (recipe = the proven R4 run).
...
MODEL=/home/valentine/models/mosaic-12l-gatea
...
if nvidia-smi --query-compute-apps=pid --format=csv,noheader | grep -q '[0-9]'; then
  ...
  docker rm -f glm53-mosaic-10lg >>"$LOG" 2>&1 || true
  sleep 5
fi
if nvidia-smi --query-compute-apps=pid --format=csv,noheader | grep -q '[0-9]'; then
