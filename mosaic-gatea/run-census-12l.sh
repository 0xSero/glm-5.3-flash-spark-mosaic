...
docker run --rm -v /home/valentine/models:/home/valentine/models:ro -v $W:$W --entrypoint python3 $IMG \
  -m exl3_plain_sglang_overlay.census $DST --out $W/census-mosaic-12l-stock.json
echo "census exit=$?"
python3 - <<'PY'
import json
r = json.load(open('/home/valentine/mosaic-gatea/census-mosaic-12l-stock.json'))
e = r.get('experts', {})
v = r.get('verdict', {})
...
PY
echo "### CENSUS_12L_DONE $(date -u +%FT%TZ)"
