...
a = p.parse_args()
assert a.attempt >= 5
NAME = f'glm53-mtp-nvfp4-component-attempt{a.attempt}'
...
assert MODEL.is_dir()
out = ROOT / f'gpu-attempt{a.attempt}'
out.mkdir(exist_ok=False)
source = out / 'source'
source.mkdir()
for script in ('policy.py', 'cpu_scope.py', 'gpu_component.py', 'launch_component.py', 'packed_routes.py', 'reference_packed.py'):
...
cid = read('docker', 'run', '-d', '--name', NAME, '--restart', 'no', '--network', 'none',
           ...
           '-e', f'GLM53_TEST_PACKED_ROUTES={int(a.packed)}',
