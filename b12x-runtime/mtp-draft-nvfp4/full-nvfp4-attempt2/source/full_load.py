...
if MODE == 'nvfp4':
    ...
    Path('/out/prepared-tensors.json').write_text(json.dumps(prepared, indent=2)+'\n')
    # B12x's MMA packing stores eight FP4 codes per INT32 word. It retains
    # four-bit storage; the canonical runtime tensor is not the input UINT8 view.
    assert values.w1_fp4.dtype == values.w2_fp4.dtype == torch.int32
    assert sum(t.numel()*t.element_size() for t in (values.w1_fp4, values.w2_fp4)) == 288*4096*2048*3//2
else:
