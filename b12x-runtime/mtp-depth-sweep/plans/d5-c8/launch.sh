...
exec docker run -d --pull never --gpus all --ipc host --network host \
  ...
  --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY","custom_ops":["all"],"cudagraph_capture_sizes":[6,12,18,24,30,36,42,48],"max_cudagraph_capture_size":48}' \
  ...
  --speculative-config '{"method":"mtp","model":"/mtp","num_speculative_tokens":5,"attention_backend":"B12X"}'
