...
def b12x_warmup(worker: "Worker", cudagraph_capture_sizes: list[int]) -> None:
    ...
    if os.environ.get("GLM53_MTP_EXPERT_NVFP4") == "1":
        # Native MTP is a separate model, outside worker.get_model().modules().
        # Its packed plans need completed eager launches before graph profiling.
        draft = worker.get_draft_model()
        if draft is None:
            raise RuntimeError("NVFP4 MTP warmup requires the loaded native draft")
        draft_units = list(_collect_warmup_units(draft, token_counts, output_dtype))
        if not any(unit.name == "MoE" for unit in draft_units):
            raise RuntimeError("NVFP4 MTP warmup found no draft MoE provider")
        units = (*units, *draft_units)
