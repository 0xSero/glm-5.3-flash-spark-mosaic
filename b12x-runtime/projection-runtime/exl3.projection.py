...
from vllm.model_executor.weight_transfer import (
    copy_weight, flush_weight_transfers, get_file_tensor_source, materialize_weight,
)
...
class Exl3MoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        k_words = self.projection_bits[0] * 16
        ...
        def new_packed() -> dict[str, Parameter]:
            return {
                ...
                "w2_trellis": Parameter(
                    torch.empty(
                        num_experts, out_tiles, in_tiles, down_k_words, dtype=torch.int16
                    ),
                    requires_grad=False,
                ),
                "w2_suh": Parameter(
    ...
    def _load_exl3(
        ...
        # Jovian can deliver a file-range descriptor without resident payload.
        # Keep rank-stacked packed inputs intact so the scoped writer can read
        # directly into their final parameter slice. Numerical/view transforms
        # need materialization; scalar MCG costs only four bytes.
        loaded = loaded_weight
        if get_file_tensor_source(loaded) is not None:
            if self.quant_config.rank_stacked_tp is None or (
                suffix == "mcg" and loaded.ndim == 0
            ):
                loaded = materialize_weight(loaded)
        else:
            loaded = loaded.detach().contiguous()
        ...
        copy_weight(dest, sharded)
        return True if return_success else None

    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        # Pointer construction and MCG validation consume loaded values.
        ...
        for e in range(n_physical):
            for w13, w2 in packed_ranks:
                ...
                if (gate.K, up.K, down.K) != self.projection_bits:
