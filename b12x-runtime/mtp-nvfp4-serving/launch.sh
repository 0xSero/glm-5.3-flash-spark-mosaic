...
exec docker run -d --pull never --gpus all --ipc host --network host \
  --name "$name" --log-driver json-file --log-opt max-size=10m --log-opt max-file=3 \
  ...
  -e GLM53_MTP_EXPERT_FP8=0 -e GLM53_MTP_EXPERT_NVFP4=1 -e VLLM_B12X_MOE_FP4_FORCE_A16=1 \
  ...
  --speculative-config '{"method":"mtp","model":"/mtp","num_speculative_tokens":1,"attention_backend":"B12X"}'
