...
for n,h in json.loads((W/'SOURCE_FREEZE_ATTEMPT2.json').read_text()).items():assert sha(W/n)==h,n
...
model=W/'flat-projection14'
mtp=ROOT/'b12x-runtime/mtp-draft-nvfp4/native-mtp-view'
assert sha(model/'model.safetensors.index.json')=='f4097a4ed3ccefbcf60800695c45f68f8dd668402cc1aae1ba71ba4578cc5988'
flatcpu=json.loads(docker('inspect','glm53-projection14-flat-view-cpu'))[0]
assert flatcpu['Image']==IMAGE and flatcpu['State']['ExitCode']==0 and not flatcpu['State']['Running'] and not flatcpu['State']['OOMKilled']
assert json.loads((W/'flat-view-cpu.log').read_text().splitlines()[-1])['passed']
...
run=W/'attempt2';run.mkdir(exist_ok=False)
...
plan={'state':'LAUNCH_ATTEMPTED_NOT_ADMITTED','model':'glm-5.3-flash','container_name':'glm53-projection14-nvfp4mtp-attempt2','image_id':IMAGE,'model_root':str(model),'mtp_root':str(mtp),'depth':1,'slots':1,
