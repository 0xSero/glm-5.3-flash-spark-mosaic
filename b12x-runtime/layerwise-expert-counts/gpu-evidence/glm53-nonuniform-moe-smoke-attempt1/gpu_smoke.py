"""Two small real-weight MoEs: counts8/16, full model geometry, no quality claim."""
import contextlib
import hashlib
import json
import os
...
from vllm.models.glm5next.nvidia.layerwise_counts import layer_config, validate_loaded_tensor
...
kept={5:list(range(32,40)),32:list(range(80,96))}
config.routed_experts_per_layer={str(k):len(v) for k,v in kept.items()}
config.retained_expert_ids_by_layer={str(k):v for k,v in kept.items()}
...
with set_current_vllm_config(c), torch.device('cuda'), contextlib.ExitStack() as stack:
    ...
    def get(name):
        path=index[name]
        if path not in handles: handles[path]=stack.enter_context(safe_open('/model/'+path,framework='pt',device='cpu'))
        return handles[path].get_tensor(name)
    for layer,ids in kept.items():
        cfg=layer_config(config,c.parallel_config,layer)
        ...
        validate_loaded_tensor(config,prefix+'gate.weight',router.shape)
        validate_loaded_tensor(config,prefix+'gate.e_score_correction_bias',bias.shape)
            ...
            for proj,packed,sid in [('gate_proj','w13','w1'),('up_proj','w13','w3'),('down_proj','w2','w2')]:
                for rank in range(4):
                    for field in ('trellis','suh','svh','mcg'):
                        ...
                        tensor=get(old)
                        validate_loaded_tensor(config,remapped,tensor.shape)
    ...
    for row,(layer,m) in zip(report['layers'],models):
        ...
        torch.testing.assert_close(fused,loop,atol=.01,rtol=.03)
        ...
        torch.testing.assert_close(fused.float(),ref,atol=.02,rtol=.05)
        ...
        row.update({'route_ids':route_ids[0].tolist(),'fused_vs_loop_max_abs':float((fused-loop).abs().max()),'fused_vs_reconstructed_fp32_max_abs':float((fused.float()-ref).abs().max()),'graph_replays':3,'graph_vs_eager_max_abs':float((output-eager).abs().max()),'finite':True})
